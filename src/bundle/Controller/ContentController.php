...
class ContentController extends Controller
{
    ...
    public function createDraftAction(Request $request): Response
    {
        ...
        $data = $form->getData();
        $contentInfo = $data->getContentInfo();
        $redirectResponse = $this->redirectToRoute('ezplatform.dashboard');

        if (null !== $contentInfo) {
            $redirectResponse = $this->redirectToRoute('_ezpublishLocation', [
                'locationId' => $data->getContentInfo()->mainLocationId,
            ]);
        }
